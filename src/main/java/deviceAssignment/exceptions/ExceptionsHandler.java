...
public class ExceptionsHandler {
    ...
    public ResponseEntity<ErrorsPayloadWithListErr> handleValidationErrors(MethodArgumentNotValidException ex) {
        List<String> errors = ex.getBindingResult().getFieldErrors().stream().map(error -> error.getDefaultMessage())
                .collect(Collectors.toList());

        ErrorsPayloadWithListErr payload = new ErrorsPayloadWithListErr("Ci sono stati errori nel body",
                new Date(), 400,
                errors);

        return new ResponseEntity<ErrorsPayloadWithListErr>(payload, HttpStatus.BAD_REQUEST);
    }
